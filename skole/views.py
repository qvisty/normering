...
def team_detail(request, team_id):
    ...
    for school_class in school_classes:
        ...
        total_price_for_class = sum(lesson['total_lessons_per_teacher'] * lesson['price_per_lesson'] for lesson in lessons)
        total_price_for_team += total_price_for_class

        # Calculate total school fee for this class (sum of school fee for each student)
        total_school_fee_for_class = sum(student.school_fee.amount for student in school_class.students.all() if student.school_fee)
    ...
    total_hours_list = [(category, hours) for category, hours in total_hours_by_category.items()]
    ...
    if total_school_fee_for_team > 0:
        percentage_used = round((total_price_for_team / total_school_fee_for_team) * 100, 1)

    return render(request, 'skole/team_detail.html', {'team': team, 'summary_data': summary_data, 'totals': totals, 
        'team': team,
